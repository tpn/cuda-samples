...
// `ipcHandleTypeFlag` specifies the platform specific handle type this sample uses for importing and
// exporting memory allocation. On Linux this sample specifies the type as CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR
// meaning that file descriptors will be used. On Windows this sample specifies the type as CU_MEM_HANDLE_TYPE_WIN32
// meaning that NT HANDLEs will be used.
// The ipcHandleTypeFlag variable is a convenience variable and is passed by value to individual requests.
...
static void
barrierWait(volatile int *barrier, volatile int *sense, unsigned int n)
{
    int count;
...
static void
memMapAllocateAndExportMemory(unsigned char backingDevice, size_t allocSize, std::vector<CUmemGenericAllocationHandle>& allocationHandles,
                              std::vector<ShareableHandle>& shareableHandles)
{
    // This property structure describes the physical location where the memory will be allocated
    // via cuMemCreate along with additional properties.
    CUmemAllocationProp prop = {};

    // The allocations will be device pinned memory backed on backingDevice and exportable with the specified handle type.
    ...
    // Passing a requestedHandleTypes indicates intention to export this allocation to a platform-specific handle.
    // This sample requests a file descriptor on Linux and NT Handle on Windows.
    ...
    // Windows-specific LPSECURITYATTRIBUTES is required when CU_MEM_HANDLE_TYPE_WIN32 is used.
    // The security attribute defines the scope of which exported allocations may be tranferred to other
    // processes. For all other handle types, pass NULL.
    getDefaultSecurityDescriptor(&prop);

    for (int i = 0; i < allocationHandles.size(); i++) {
        // Create the allocation as a pinned allocation on device specified in prop.location.id
        checkCudaErrors(cuMemCreate(&allocationHandles[i], allocSize, &prop, 0));

        // Export the allocation to a platform-specific handle. The type of handle requested here must match the
        // requestedHandleTypes field in the prop structure passed to cuMemCreate.
        checkCudaErrors(cuMemExportToShareableHandle((void *)&shareableHandles[i], allocationHandles[i], ipcHandleTypeFlag, 0));
    }
}

static void
memMapImportAndMapMemory(CUdeviceptr d_ptr, size_t mapSize, std::vector<ShareableHandle>& shareableHandles, int mapDevice)
{
    std::vector<CUmemGenericAllocationHandle> allocationHandles;
    allocationHandles.resize(shareableHandles.size());

    // The accessDescriptor will describe the mapping requirement for the mapDevice passed as argument
    ...
    for (int i = 0; i < shareableHandles.size(); i++) {
        // Import the memory allocation back into a CUDA handle from the platform specific handle.
...
static void
memMapUnmapAndFreeMemory(CUdeviceptr dptr, size_t size)
{
...
static void
memMapGetDeviceFunction(char **argv)
{
...
static void childProcess(int id, char **argv)
{
    ...
    // Import the memory allocations shared by the parent with us and map them in our address space.
    memMapImportAndMapMemory(d_ptr, DATA_BUF_SIZE, shHandle, id);

    // Since we have imported allocations shared by the parent with us, we can close all the ShareableHandles.
    ...
    for (int i = 0; i < procCount; i++) {
        ...
        // before proceeding to the next. This makes the data in the buffer deterministic.
    ...
    // Copy the data onto host and verify value if it matches expected value or not.
    ...
    // Unmap the allocations from our address space. Unmapping will also free the handle as we have already released the imported
    // handle with the call to cuMemRelease. Finally, free up the Virtual Address space we reserved with cuMemAddressReserve.
...
static void parentProcess(char *app)
{
    ...
    for (i = 0; i < devCount; i++) {
        ...
        checkCudaErrors(cuDeviceGetAttribute(&attributeVal, CU_DEVICE_ATTRIBUTE_VIRTUAL_ADDRESS_MANAGEMENT_SUPPORTED, devices[i]));
#if defined(__linux__)
        checkCudaErrors(cuDeviceGetAttribute(&deviceSupportsIpcHandle, CU_DEVICE_ATTRIBUTE_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR_SUPPORTED, devices[i]));
#else
    ...
    // Allocate `nprocesses` number of memory chunks and obtain a shareable handle for each
    // allocation. Share all memory allocations with all children.
